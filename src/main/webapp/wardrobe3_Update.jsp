...
// 更新資料
String sql = "UPDATE my_wardrobe SET text_description = ?, brand = ?, pic = ?, color_code = ?, size = ? WHERE clothing_number = ? AND memberId = ? AND clothing_code = ?";
...
// ✅ 驗證更新結果
if(result > 0) {
    String verifySql = "SELECT text_description, brand, pic, color_code, size FROM my_wardrobe WHERE clothing_number = ? AND memberId = ? AND clothing_code = ?";
    ...
    if(verifyRs.next()) {
        ...
        System.out.println("size: " + verifyRs.getString("size"));
    }
    verifyRs.close();
    verifyStmt.close();
}
